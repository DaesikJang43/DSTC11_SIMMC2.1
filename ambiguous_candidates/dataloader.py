...
class TrainDataset_1(Dataset):
    def __init__(self, args, features, obj_embed=None, scene_embed=None, bbox_embed=None, obj_text=None):
        self.len = len(features['input_ids'])
        self.features = features
        self.negative_sample_size = args.negative_sample_size
        self.obj_embed = obj_embed if obj_embed is not None else None
        self.scene_embed = scene_embed if scene_embed is not None else None
        self.bbox_embed = bbox_embed if bbox_embed is not None else None
        self.obj_text = obj_text if obj_text is not None else None
    ...
    def __getitem__(self, idx):
        ...
        if self.scene_embed is not None:
        ...
        return input_ids, attention_mask, token_type_ids, scene_embedding, obj_embedding, labels, obj_bbox, obj_input_ids, obj_attention_mask, obj_token_type_ids
...
class TrainDataset_2(Dataset):
    def __init__(self, args, features, obj_embed, scene_embed, bbox_embed, obj_text=None):
        self.len = len(features['input_ids'])
        self.features = features
        self.negative_sample_size = args.negative_sample_size
        self.obj_embed = obj_embed if obj_embed is not None else None
...
class TestDataset(Dataset):
    ...
    def collate_fn(data):
        ...
        return input_ids, attention_mask, token_type_ids, scene_embedding, obj_embedding, dialog_id, turn_id, object_map, obj_bbox, \
            obj_input_ids, obj_attention_mask, obj_token_type_ids